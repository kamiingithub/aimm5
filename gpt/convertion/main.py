...
# 设置页面标题
st.set_page_config(page_title="😈数据变形器🧚‍", page_icon="🧚‍", layout="wide")
st.title("😈数据变形器🧚‍")
...
def chatgpt_response(type, user_message):
    messages = [
        HumanMessage(content="请将:::后的内容转换成{}:::".format(type) + user_message)
...
option = st.selectbox(
    'choose a type what you want to convert',
    ('json', 'execel', 'java properties', 'mysql ddl'))
